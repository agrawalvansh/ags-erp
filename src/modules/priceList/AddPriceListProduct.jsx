...
const AddPriceListProduct = () => {
  ...
  useEffect(() => {
    if (editing || !formData.productName || !formData.size) return;
    ...
    const code = `${slugify(formData.productName)}-${slugify(formData.size)}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || `Failed to ${editing ? 'update' : 'save'} product`);
      }
